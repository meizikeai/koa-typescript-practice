...
export default class BaseController {
  ...
  // 403
  forbidden = async (ctx: AppContext) => {
    const accepts = ctx.accepts('html')
    if (accepts === 'html') {
  ...
  // Authorization
  auth = async (ctx: AppContext, next: Next) => {
    ...
    let claims: string | jwt.JwtPayload | null = null
    ...
    if (!claims || typeof claims === 'string' || claims.exp! < nowInSeconds || claims.iat! > nowInSeconds) {
      ctx.status = 401
      ctx.body = {
        code: 401,
